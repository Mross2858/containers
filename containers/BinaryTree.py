...
These two classes are the building blocks for the BST, AVLTree, and Heap data
structures.
...
class Node():
    ...
    Given a node t, you can visualize the node by running str(t) in the python
    interpreter.
...
class BinaryTree():
    ...
    def print_tree(self, traversal_type):
        '''
        There are three primary types of tree traversals: preorder, inorder,
        and postorder.
        ...
        if traversal_type == 'preorder':
            ...
            raise ValueError('Traversal type ' + str(traversal_type) + 'not supported.')
    ...
    def to_list(self, traversal_type):
        ...
        A general programming principle is that a function should return its
        results
        ...
        But by returning the results we can also do more computations on the
        results if needed.
    ...
    def __len__helper(node):
        ...
        if a left child exists, add the result of __len__helper on the left
        child;
        if a right child exists, add the result of __len__helper on the right
        child;
        ...
        if not node:
            ...
            return 1 + BinaryTree.__len__helper(node.right) + BinaryTree.__len__helper(node.left)

    def height(self):
        '''
        Returns the height of the tree.
        Recall that the height is the maximum length from the root to a leaf
        node.
